    ...
    <div class="navbar-menu" :class="{'is-active': showMenu}">
      ...
      <form class="navbar-end" @submit.prevent="search">
        <div class="navbar-item input-item">
          ...
          <button type="submit" class="button is-ncc" :class="{'is-loading': searching}">
            <Icon icon="search" />
          </button>
        </div>
      </form>
...
<script>
  import {get as nccinfoGet} from '@/services/nccinfo-api'
  ...
  export default {
    ...
    methods: {
      async search() {
        ...
        this.searching = true
        try {
          let {type, id, address} = await nccinfoGet(`/search`, {params: {query: searchString}})
          switch (type) {
          case 'address':
            this.searchString = ''
